 ...
 * the public renderer. Returns `null` for:
 *  - unknown block types (the registry filters them out)
 *  - rows whose `data` payload is missing required fields for the
 *    declared type (defensive: we'd rather drop one block than
 *    crash the whole page)
 *
 * Extracted from `resolve-page.ts` in step 45 so the preview
 * renderer can reuse the exact same mapping for draft blocks
 * pulled from a cookie. Touching this file changes both the
 * public site and the preview iframe in lock-step.
 */
export function toContentBlock(row: Block): ContentBlock | null {
